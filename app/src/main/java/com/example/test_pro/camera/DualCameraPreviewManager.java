...
public class DualCameraPreviewManager {
    ...
    public DualCameraPreviewManager(@NonNull Context context, TextureView tvRGB, TextureView tvIR, FrameCallback callback) {
    ...
    public void stop() {
        if (cameraDeviceRGB != null) cameraDeviceRGB.close();
        if (cameraDeviceIR != null) cameraDeviceIR.close();
        if (imageReaderRGB != null) imageReaderRGB.close();
        if (imageReaderIR != null) imageReaderIR.close();
        stopBackgroundThread();
    }

    private void setupTextureListener(@NonNull TextureView textureView, String cameraId, boolean isFirst) {
        if (textureView.isAvailable()) {
            openCamera(cameraId, textureView, isFirst);
        } else {
    ...
    private void openCamera(String cameraId, TextureView view, boolean isFirst) {
        try {
            ...
            cameraManager.openCamera(cameraId, new CameraDevice.StateCallback() {
                ...
                public void onOpened(@NonNull CameraDevice camera) {
                    if (isFirst) {
                        ...
                        setupPreview(cameraDeviceRGB, view, cameraId);
                    } else {
                        cameraDeviceIR = camera;
                        setupPreview(cameraDeviceIR, view, cameraId);
                    }
                }
    ...
    private void setupPreview(CameraDevice device, @NonNull TextureView textureView, @NonNull String cameraId) {
        try {
            ...
            if (ConstantString.CAMERA_BACK_ID.equals(cameraId)) {
                ...
                previewSize = new Size(NumericConstants.WIDTH_720, NumericConstants.HEIGHT_480);
            }
            ...
            if (cameraId.equals(ConstantString.CAMERA_BACK_ID)) imageReaderRGB = imageReader;
    ...
    @NonNull
    private byte[] YUV_420_888_TO_NV21(@NonNull Image image) {
